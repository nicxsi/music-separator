...
class FileRepository(IFileRepository):
    """
    Implementation of a repository for working with a local file system
    A component of the infrastructure layer. Responsible for physically
    saving downloaded audio files, calculating paths to sources and results,
    and packaging split Demucs tracks into ZIP archives.
    """
    def __init__(self, upload_dir: Path, output_dir: Path):
        self.upload_dir = upload_dir
        self.output_dir = output_dir

    def save_upload(self, file_stream: BinaryIO, filename: str, job_id: str) -> Path:
        file_path = self.upload_dir / f"{job_id}_{filename}"

        self._save_file(file_stream, file_path)
